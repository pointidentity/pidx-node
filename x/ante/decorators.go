...
// related transactions incur fixed fee cost. Fixed fee is seperate for each x module transactions and are updated through Governance
...
func (mfd DeductFeeDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (newCtx sdk.Context, err error) {
	...
	// If present, calculate the total fee of all fixed-cost x messages
	if len(ssiMsgs) > 0 {
		...
		// If there is atleast one x  message, check if the fee provided meets the requirement for the fixedSSIFee by asserting if the former
		...
		if !fee.IsEqual(fixedSSIFee) {
			errMsg1 := "the transaction consists of x module based messages which incur fixed cost. "
			errMsg2 := "The fee provided MUST BE equal to the sum of all fixed-fee x messages. "
			errMsg3 := "To know about the fixed-fee cost of all x transactions, refer the API endpoint /pointidentity/pidxnode/fixedfee . "
