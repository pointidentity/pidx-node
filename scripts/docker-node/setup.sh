...
cat /root/.pidx-node/config/genesis.json | jq '.app_state["staking"]["params"]["bond_denom"]="pidx"' > /root/.pidx-node/config/tmp_genesis.json && mv /root/.pidx-node/config/tmp_genesis.json /root/.pidx-node/config/genesis.json
