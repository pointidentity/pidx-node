...
SOURCE_ADDRESS=$(pidx-noded keys show node1 -a)
...
sign_and_broadcast() {
    ...
    echo ""
    pidx-noded tx sign ./multiple_bank_msgs.json --from ${1} --chain-id pidxnode > signed_tx.json
    echo ""
    ...
    echo ""
    BROADCAST=$(pidx-noded tx broadcast ./signed_tx.json --broadcast-mode block --output json)
}

# Generate Tx
rm -rf ./multiple_bank_msgs.json
pidx-noded tx bank send ${SOURCE_ADDRESS} ${DESTINATION_ADDRESS} 10upidx --chain-id pidxnode --generate-only > multiple_bank_msgs.json
