...
IS_JAILED=$(pidx-noded q staking validators --home $HOME/.pidx-node/val1 --node tcp://localhost:36657 --output json | jq '.validators' | jq -c '.[] | select(.delegator_shares=="'"${VALIDATOR_DELEGATED_SHARES}"'")' | jq '.jailed')
