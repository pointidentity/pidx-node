...
echo ""
# Node Parameters
OLD_PIDXNODE_BINARY=./binaries/pidx-noded-old
NEW_PIDXNODE_BINARY=./binaries/pidx-noded-new
BLOCK_PERIOD=5
...
echo ""
../../../../scripts/localnet-single-node/setup.sh ${OLD_PIDXNODE_BINARY}

# Create Cosmovisor directory
export DAEMON_NAME=pidx-noded
export DAEMON_HOME=$HOME/.pidx-node
mkdir -p $DAEMON_HOME/cosmovisor/genesis/bin
cp ${OLD_PIDXNODE_BINARY} $DAEMON_HOME/cosmovisor/genesis/bin/pidx-noded
...
sleep 6
node_run_check ${OLD_PIDXNODE_BINARY}
if [[ -n $(${OLD_PIDXNODE_BINARY} status) ]]; then
    echo "pidx-noded daemon is now running"
    echo ""
else
    echo "pidx-noded daemon failed to start, exiting...."
...
echo ""
${OLD_PIDXNODE_BINARY} tx gov submit-proposal software-upgrade "${PROPOSAL_NAME}" --title "${PROPOSAL_TITLE}" --description "${PROPOSAL_DESCRIPTION}" --from node1 --upgrade-height "${UPGRADE_HEIGHT}" --deposit "${DEPOSIT}" --chain-id pidxnode --broadcast-mode block --yes
echo ""
...
echo ""
${OLD_PIDXNODE_BINARY} tx gov vote 1 yes --from node1 --chain-id pidxnode --broadcast-mode block --yes
echo ""
echo "Vote Given"
echo ""
...
echo ""
mkdir -p $HOME/.pidx-node/cosmovisor/upgrades/test/bin
cp ${NEW_PIDXNODE_BINARY} $HOME/.pidx-node/cosmovisor/upgrades/test/bin/pidx-noded

LATEST_BLOCK_HEIGHT=$(pidx-noded status | jq '.SyncInfo["latest_block_height"]' | bc)
...
if [[ -n $(${NEW_PIDXNODE_BINARY} status) ]]; then
    echo "pidx-noded daemon is now running"
    echo ""
else
    echo "pidx-noded daemon failed to start, exiting...."
