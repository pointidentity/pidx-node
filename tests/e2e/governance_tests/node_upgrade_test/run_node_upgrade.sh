...
echo ""
# Node Parameters
OLD_HIDNODE_BINARY=./binaries/pidx-noded-old
NEW_HIDNODE_BINARY=./binaries/pidx-noded-new
BLOCK_PERIOD=5
...
cp ${OLD_HIDNODE_BINARY} $DAEMON_HOME/cosmovisor/genesis/bin/pidx-noded
...
echo ""
${OLD_HIDNODE_BINARY} tx gov submit-proposal software-upgrade "${PROPOSAL_NAME}" --title "${PROPOSAL_TITLE}" --description "${PROPOSAL_DESCRIPTION}" --from node1 --upgrade-height "${UPGRADE_HEIGHT}" --deposit "${DEPOSIT}" --chain-id pidxnode --broadcast-mode block --yes
echo ""
...
echo ""
${OLD_HIDNODE_BINARY} tx gov vote 1 yes --from node1 --chain-id pidxnode --broadcast-mode block --yes
echo ""
echo "Vote Given"
echo ""
...
cp ${NEW_HIDNODE_BINARY} $HOME/.pidx-node/cosmovisor/upgrades/test/bin/pidx-noded
